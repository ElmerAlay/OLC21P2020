...
public class AND implements ASTNode{
    ...
    public AND(ASTNode op1, ASTNode op2, int row, int column) {
        super();
        this.op1 = op1;
        this.op2 = op2;
        this.row = row;
    ...
    public Object execute(Environment environment, LinkedList<TError> LError) {
        ...
        if(op1 instanceof Vec && op2 instanceof Vec){
            ...
            if (vec1.length==1 && vec2.length==1) {
                if(vec1[0] instanceof Boolean && vec2[0] instanceof Boolean){
            ...
            //Verifico que los 2 vectores sean del mismo tamaño pero que no sean 1
                ...
                for(int i=0; i<vec1.length; i++){
                    if(vec1[i] instanceof Boolean && vec2[i] instanceof Boolean){
            ...
            //verifico que el 1er vector sea 1 y el segundo sea de mayor tamaño
                ...
                for(int i=0; i<vec2.length; i++){
                    if(vec1[0] instanceof Boolean && vec2[i] instanceof Boolean){
            ...
            //Verifico que el primer vector sea de tamaño n y el vector 2 sea tamaño 1
                ...
                //Recorrro los vectores y opero
                for(int i=0; i<vec1.length; i++){
                    if(vec1[i] instanceof Boolean && vec2[0] instanceof Boolean){
            ...
            if(mat1.row==mat2.row && mat1.col==mat2.col){
                ...
                Object res = new AND(new Constant(new Vec(o1)), new Constant(new Vec(o2)), row, column).execute(environment, LError);
            ...
            if(vec.length==1){
                ...
                Object res = new AND(new Constant(new Vec(o1)), new Constant((Vec)op2), row, column).execute(environment, LError);
                Object result[][] = new Object[mat1.row][mat1.col];
                con1 = 0;
                if(res instanceof Vec){
                    for(int i=0;i<mat1.col;i++){
            ...
            if(vec.length==1){
                ...
                Object res = new AND(new Constant((Vec)op1), new Constant(new Vec(o1)), row, column).execute(environment, LError);
                Object result[][] = new Object[mat1.row][mat1.col];
                con1 = 0;
                if(res instanceof Vec){
                    for(int i=0;i<mat1.col;i++){

...
public class Multiplication implements ASTNode{
    ...
    public Multiplication(ASTNode op1, ASTNode op2, int row, int column) {
        super();
        this.op1 = op1;
        this.op2 = op2;
        this.row = row;
        ...
        if(op1 instanceof Vec && op2 instanceof Vec){
            ...
            if (vec1.length==1 && vec2.length==1) {
                if((vec1[0] instanceof Double && (vec2[0] instanceof Double || vec2[0] instanceof Integer)) || 
                    ...
                    TError error = new TError("*", "Semántico", "no se puede multiplicar esos 2 tipos de datos", row, column);
                    LError.add(error);
            ...
            //Por último es el caso en el que los vectores son de distinto tamaño
                ...
                TError error = new TError("*", "Semántico", "no se puede multiplicar esos 2 vectores de distinto tamaño", row, column);
                LError.add(error);
            ...
            if(mat1.row==mat2.row && mat1.col==mat2.col){
                ...
                Object res = new Multiplication(new Constant(new Vec(o1)), new Constant(new Vec(o2)), row, column).execute(environment, LError);
                ...
                if(res instanceof Vec){
                    ...
                    TError error = new TError("*", "Semántico", "Error al multiplicar las matrices", row, column);
                    LError.add(error);

                    return error;
                }
            }else{
                TError error = new TError("*", "Semántico", "no se puede multiplicar las matrices porque no tienen las mismas dimensiones", row, column);
                LError.add(error);
            ...
            if(vec.length==1){
                ...
                Object res = new Multiplication(new Constant(new Vec(o1)), new Constant((Vec)op2), row, column).execute(environment, LError);
                Object result[][] = new Object[mat1.row][mat1.col];
                con1 = 0;
                if(res instanceof Vec){
                    for(int i=0;i<mat1.col;i++){
                    ...
                    TError error = new TError("*", "Semántico", "Error al multiplicar la matriz con el vector", row, column);
                    LError.add(error);

                    return error;
                }
            }else{
                TError error = new TError("*", "Semántico", "no se puede multiplicar una matriz y un vector de más de un valor", row, column);
                LError.add(error);
            ...
            if(vec.length==1){
                ...
                Object res = new Multiplication(new Constant((Vec)op1), new Constant(new Vec(o1)), row, column).execute(environment, LError);
                Object result[][] = new Object[mat1.row][mat1.col];
                con1 = 0;
                if(res instanceof Vec){
                    for(int i=0;i<mat1.col;i++){

 ...
 * @author junio
 */
public class MatAssig3 implements ASTNode{
    private String name;
    ...
    public MatAssig3(String name, ASTNode ind1,ASTNode exp, int row, int column) {
        super();
        this.name = name;
        this.ind1 = ind1;
        this.exp = exp;
    ...
    public Object execute(Environment environment, LinkedList<TError> LError) {
        ...
        if(environment.get(name) != null && environment.get(name).getValue() instanceof Mat){
            ...
            if(op1 instanceof Vec && ((Vec)op1).getValues().length==1 &&
                    ((Vec)op1).getValues()[0] instanceof Integer && Integer.parseInt((((Vec)op1).getValues()[0]).toString())<=mat.col &&
                ...
                if(op instanceof Vec && ((Vec)op).getValues().length==1){
                    ...
                    for(int i=0; i<mat.row; i++){
                        int index = SupportFunctions.mapLexiMat(i, col-1, mat.row);
                ...
                else if(op instanceof Vec && ((Vec)op).getValues().length==mat.row){
                    int col = Integer.parseInt((((Vec)op1).getValues()[0]).toString());
                    Object vals[] = new Object[mat.row*mat.col];
                    int cont = 0;
                    for(int i=0; i<mat.col; i++){

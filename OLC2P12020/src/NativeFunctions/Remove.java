...
public class Remove implements ASTNode{
    ...
    public Remove(ASTNode exp1, ASTNode exp2) {
        super();
        this.exp1 = exp1;
        this.exp2 = exp2;
    }
    ...
    public Object execute(Environment environment, LinkedList<TError> LError) {
        ...
        if(op1 instanceof Vec && op2 instanceof Vec){
            ...
            if(values.length==1 && values2.length==1){
                ...
                if(values[0] instanceof String && values2[0] instanceof String){
                    ...
                    TError error = new TError("remove", "Semántico", "Los vectores dentro de la función no son de tipo String", 0, 0);
                    LError.add(error);

                    return error;
                }
            }else{
                TError error = new TError("remove", "Semántico", "Los vectores dentro de la función deben ser tamaño 1", 0, 0);
                LError.add(error);
        ...
        TError error = new TError("remove", "Semántico", "La expresión dentro de la función no es válida", 0, 0);
        LError.add(error);
